...
const EmotionDetector: React.FC<EmotionDetectorProps> = ({ onEmotionDetected, isActive }) => {
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        setPermissionError('Failed to load facial recognition models. Please refresh and try again.');
        setIsInitializing(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const startVideo = async () => {
      try {
        ...
        setPermissionError('Unable to access your camera. Please grant permission and refresh.');
      }
    };
  ...
  useEffect(() => {
    if (!isModelLoaded || !isActive || !videoRef.current || permissionError) return;
    ...
    const expressionToEmotion = (expressions: faceapi.FaceExpressions): { emotion: Emotion, confidence: number } => {
  ...
  if (isInitializing) {
    return (
      <div className="flex flex-col items-center justify-center h-48 md:h-64 lg:h-80 bg-gray-800 rounded-xl p-4">
        ...
        <p className="text-white text-center">Initializing emotion detection...</p>
      </div>
    );
  }
